...
public class DriverFactory {
    ...
    private static final String SMART_WAIT_ADDON_CHROME_RELATIVE_PATH = File.separator + "Chrome" + File.separator
            + "Smart Wait";

    private static final String SMART_WAIT_ADDON_FIREFOX_RELATIVE_PATH = File.separator + "Firefox" + File.separator
            + "smartwait.xpi";
    ...
    private static boolean isEdgeBrowser(DesiredCapabilities desiredCapibilities) {
        ...
        if (capabilityNames.contains(StringConstants.BROWSER_CAPABILITY_NAME)
                && desiredCapibilities.getCapability(StringConstants.BROWSER_CAPABILITY_NAME) instanceof String) {
            return ((String) desiredCapibilities.getCapability(StringConstants.BROWSER_CAPABILITY_NAME)).toLowerCase()
                    .contains("edge");
        }
        return false;
    }
    ...
    private static WebDriver createNewRemoteChromeDriver(DesiredCapabilities desireCapibilities)
        ...
        Runtime.getRuntime()
                .exec(new String[] { System.getProperty(CHROME_DRIVER_PATH_PROPERTY_KEY),
    ...
    private static WebDriver createNewFirefoxDriver(DesiredCapabilities desiredCapabilities) {
        ...
        if (firefoxMajorVersion >= USING_GECKO_VERSION) {
            setFirefoxBinaryForMac(desiredCapabilities);
    ...
    private static void setFirefoxBinaryForMac(DesiredCapabilities desiredCapabilities) {
        if (Platform.getCurrent().is(MAC)) {
            desiredCapabilities.setCapability(FirefoxDriver.BINARY, FirefoxExecutable.MAC_FIREFOX_BINARY_FILE_LOCATION);
        }
    }
    ...
    private static DesiredCapabilities addSmartWaitExtensionToEdgeChromium(DesiredCapabilities desiredCapabilities) {
        boolean isInstalledKatalonCompactUtility = WebDriverPropertyUtil
                .didUserProfileInstallKatalonCompactUtility(WebUIDriverType.EDGE_CHROMIUM_DRIVER, desiredCapabilities);
        if (isInstalledKatalonCompactUtility) {
        ...
        if (StringUtils.isNotEmpty(serverUrl) && webDriver instanceof RemoteWebDriver) {
            ...
            if (driverPreferenceProps != null) {
                ...
                if (testCloudRemoteOS != null) {
                    ...
                    if (getBrowserVersion(webDriver).split(" ").length > 1) {
                        driverCapabilitiesMapAsDisplay.put("browserVersion",
                                getBrowserVersion(webDriver).split(" ")[1]);
                    }
                }
            }
            if (testCloudRemoteOS != null) {
    ...
    public static WebDriver openWebDriver(DriverType driver, Object options) throws Exception {
        try {
            ...
            switch (webUIDriver) {
                case FIREFOX_DRIVER:
                    if (options instanceof FirefoxProfile firefoxProfile) {
                        DesiredCapabilities desiredCapabilities = DesiredCapabilities.firefox();
                        desiredCapabilities.setCapability(FirefoxDriver.PROFILE, firefoxProfile);
                        webDriver = createNewFirefoxDriver(desiredCapabilities);
                    } else if (options instanceof DesiredCapabilities desiredCapabilities) {
                        System.setProperty("webdriver.gecko.driver", DriverFactory.getGeckoDriverPath());
                        setFirefoxBinaryForMac(desiredCapabilities);
                        webDriver = new CFirefoxDriver(GeckoDriverService.createDefaultService(), desiredCapabilities);
                    } else {
    ...
    private static String getEdgeChromiumDriverPath() {
        if (OSUtil.isWindows()) {
            ...
            File customDriverLocationLinux = new File(RunConfiguration.getProjectDir(),
                    "Include/drivers/edgechromiumdriver_linux64/msedgedriver");
            if (customDriverLocationLinux.exists()) {
                String customDriverPath = customDriverLocationLinux.getAbsolutePath();
                try {
                    logger.logInfo("Custom Edge Chromium driver detected at location: "
                            + customDriverLocationLinux.getAbsolutePath());
                    FileExcutableUtil.makeFileExecutable(customDriverPath);
                } catch (IOException e) {
                    logger.logInfo("Cannot make Edge Chromium driver file : " + customDriverPath + " excutable");
                }
